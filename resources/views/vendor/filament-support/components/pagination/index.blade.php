...
@php
    ...
    // Custom number formatter to bypass missing 'intl' extension
    if (! function_exists('fja_number_format')) {
        function fja_number_format($number) {
        ...
        <span class="fi-pagination-overview">
            {{
                trans_choice(
                    ...
                    $paginator->total(),
                    [
                        'first' => fja_number_format($paginator->firstItem() ?? 0),
                        'last' => fja_number_format($paginator->lastItem() ?? 0),
                        'total' => fja_number_format($paginator->total()),
                    ],
                )
            }}
        </span>
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-filament::pagination.item
                            ...
                            :aria-label="trans_choice('filament::components/pagination.actions.go_to_page.label', $page, ['page' => fja_number_format($page)])"
                            :label="fja_number_format($page)"
